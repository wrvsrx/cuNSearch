...
auto main() -> int {
  ...
  auto &a = nsearch.pointSets[0].neighbors[0];
  auto const secondNeighborHost = NeighborsHost{
      thrust::host_vector<uint32_t>(a.Offsets,
                                    a.Offsets + a.ParticleCountAllocationSize),
      thrust::host_vector<uint32_t>(a.Counts,
                                    a.Counts + a.ParticleCountAllocationSize),
      thrust::host_vector<uint32_t>(
          a.Neighbors, a.Neighbors + a.NeighborCountAllocationSize),
  };
